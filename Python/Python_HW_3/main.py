...
#  (“Переменная item_1 меньше”, comp_item), иначе, вывести в консоль
#  (“Переменная item_1 больше или равна”, comp_item)
if item_1 < comp_item:
...
# #                     то в теле этого if в значении переменной currency_result высчитать сколько
# #                     долларов получится при target_currency_amount и usd_eur_rate. Результат вывести
# #                     в консоль (target_currency_amount, eur_item, “=”, currency_result, usd_item)
...
# #                     то в теле этого if в значении переменной currency_result высчитать сколько
# #                     долларов получится при target_currency_amount и usd_uah_rate. Результат вывести
# #                     в консоль (target_currency_amount, uah_item, “=”, currency_result, uah_item)
...
# #                     31.7 Последним оставить else, при выполнений которого в консоль выведется
# #                     (“Unknow currency”)
# else:
...
if currency_convertor:
    currency_usd = usd_item
    target_currency = input('Валюта: ')
    target_currency_amount = input('Сумма: ')
    currency_result = 0

    if target_currency == 'eur':
        currency_result = float(target_currency_amount) / float(usd_eur_rate)
        print(target_currency_amount, eur_item, '=', currency_result, usd_item)
    elif target_currency == 'uah':
        currency_result = float(target_currency_amount) / float(usd_uah_rate)
        print(target_currency_amount, uah_item, '=', currency_result, usd_item)
    elif target_currency == 'chf':
        currency_result = float(target_currency_amount) / float(usd_chf_rate)
        print(target_currency_amount, chf_item, '=', currency_result, usd_item)
    elif target_currency == 'rub':
        currency_result = float(target_currency_amount) / float(usd_rub_rate)
        print(target_currency_amount, rub_item, '=', currency_result, usd_item)
    elif target_currency == 'byn':
        currency_result = float(target_currency_amount) / float(usd_byn_rate)
        print(target_currency_amount, byn_item, '=', currency_result, usd_item)
    elif target_currency == 'usd':
        currency_result = float(target_currency_amount) / float(usd_usd_rate)
